...
// POST — discard a pending typed draft (deletes the Tier-1 row outright).
export async function POST(
  ...
  const resp = await fetch(
    `${backend}/apollo/authored-sets/${encodeURIComponent(set_id)}/problems/${encodeURIComponent(problem_id)}/discard`,
